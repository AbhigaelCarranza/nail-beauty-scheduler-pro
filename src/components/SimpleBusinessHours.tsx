...
interface SimpleBusinessHoursProps {
  initialHours?: any[];
  onSave: (hoursData: DayHours[]) => void;
  isLoading: boolean;
}

const SimpleBusinessHours = ({ initialHours = [], onSave, isLoading }: SimpleBusinessHoursProps) => {
  // Estado predecible - siempre un objeto con keys 0-6
  ...
  useEffect(() => {
    if (initialHours && initialHours.length > 0) {
      const newState: BusinessHoursState = { ...businessHours };
      ...
      setBusinessHours(newState);
    }
  }, [initialHours]);

  // Función simple para actualizar un día específico
  ...
  // Convertir a array solo para guardar
  const handleSave = () => {
    const hoursArray = Object.values(businessHours);
    onSave(hoursArray);
  };
...
export default SimpleBusinessHours;